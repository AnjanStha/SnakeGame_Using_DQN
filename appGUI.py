...
def check_speed():
    ...
    for key in keys:
        if keys[pygame.K_SPACE] and speed_up == True:
            ...
            speed_up = False
            return
        elif keys[pygame.K_k] and speed_up == False:
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption("SnakeQ by Anjan")
    ...
    while True:
        ...
        # draw pygame GUI
        pygame.display.set_caption(f"Snake AI-Bot        Generation: {generation}     Score: {score}   ")
        win.draw_sensor_bg()
        win.draw_sensors(state_)
        win.draw_board(board, dqn.agent.env.blocks)
        win.draw_text([score, generation, steps])
